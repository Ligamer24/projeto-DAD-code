<template>
  <div v-if="game.gameEnded || match.status === 'finished'"
    ...
    <div
      ...
      <div
        ...
        <div class="mt-6 px-6">
          <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Match History</h3>
          <div class="space-y-3 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            <div v-for="game in match.gamesHistory" :key="game.roundNumber"
              ...
              <div class="absolute left-0 top-0 bottom-0 w-1.5"
                :class="game.winner ? (game.winner === auth.currentUser.id ? 'bg-green-500' : 'bg-red-500') : 'bg-gray-500'">
              </div>
              <div class="pl-3">
                  ...
                  <div class="flex items-center gap-2">
                    <span v-if="game.scoreDetail.player1 >= 61"
                      class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-gray-200 text-gray-600">{{
                        game.scoreDetail.player1 < 61 ? '' : (game.scoreDetail.player1 < 91 ? 'Risca' :
                          (game.scoreDetail.player1 < 120 ? 'Capote' : 'Bandeira')) }}</span>
                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-gray-200 text-gray-600">{{
                          game.marksAwarded.player1 }} pts</span>
                        <span class="text-xs font-black uppercase"
                          :class="game.winner ? (game.winner === auth.currentUser.id ? 'text-green-600' : 'text-red-600') : 'text-gray-600'">{{
                            game.winner ? (game.winner === auth.currentUser.id ? "WIN" : "LOSE") : "DRAW" }}</span>
                  </div>
                </div>
                <div class="grid grid-cols-3 items-center text-center">
                  <div class="flex flex-col"><span class="text-[10px] uppercase font-bold text-gray-400">Me</span><span
                      ...
                      :class="game.scoreDetail.player1 > game.scoreDetail.player2 ? 'text-gray-800' : 'text-gray-400'">{{
                        game.scoreDetail.player1 }}</span></div>
                  ...
                  <div class="flex flex-col"><span class="text-[10px] uppercase font-bold text-gray-400">Bot</span><span
                      ...
                      :class="game.scoreDetail.player2 > game.scoreDetail.player1 ? 'text-gray-800' : 'text-gray-400'">{{
                        game.scoreDetail.player2 }}</span></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="p-6 space-y-3 bg-white">
          <button v-if="match.status === 'ongoing'" @click="game.startNewGame()"
      ...
      <div class="flex flex-col w-full md:w-72 bg-amber-50 rounded-b-2xl md:rounded-r-2xl md:rounded-l-none md:mt-0 border
  ...
  <div v-else class="flex flex-col justify-between p-3 box-border h-dvh w-full" ref="gameDiv">
    ...
    <section class="flex flex-wrap gap-2 justify-center p-4 md:pb-8 pb-24">
      <GameCard v-for="(card, i) in game.player1Hand" :key="'bottom-' + i" :card="card"
        :is-interactive="game.currentTurn === auth.currentUser.id && game.tableCards.length < 2"
        @card-click="handlePlayCard(card, i)" class="transition-transform duration-200"
