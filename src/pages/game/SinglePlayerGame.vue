  ...
  >
    ...
    >
      <div class="p-8 text-center" :class="headerBgClass">
        ...
        <h2
          class="text-3xl font-black text-white uppercase tracking-wider drop-shadow-md"
        >
          {{ headerTitle }}
        </h2>
      ...
      <div class="p-6 bg-gray-50 border-b border-gray-200">
        <div class="flex justify-between items-center px-4">
          <div class="flex flex-col items-center">
            ...
            >
            <span
              class="text-5xl font-black transition-all"
              :class="
          ...
          <div class="flex flex-col items-center">
            ...
            <span
              class="text-5xl font-black transition-all"
              :class="
                match.scores.player2 > match.scores.player1
        ...
        <div
          class="mt-4 w-full h-2 bg-gray-200 rounded-full overflow-hidden flex"
        >
          <div
            class="h-full bg-green-500 transition-all duration-500"
            :style="{ width: (match.scores.player1 / 4) * 50 + '%' }"
          ></div>
          <div class="h-full bg-transparent flex-grow"></div>
          <div
            class="h-full bg-red-500 transition-all duration-500"
            :style="{ width: (match.scores.player2 / 4) * 50 + '%' }"
          ></div>
        </div>
      </div>
      ...
      >
        ...
        <span
          :class="game.scores.player1 > 60 ? 'text-green-600 font-bold' : ''"
          >You {{ game.scores.player1 }}</span
        >
        <span>-</span>
        <span :class="game.scores.player2 > 60 ? 'text-red-600 font-bold' : ''"
          >Bot {{ game.scores.player2 }}</span
        >
      </div>
      ...
      <div class="mt-6">
        ...
        <div class="space-y-3 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
          <div
            v-for="game in match.gamesHistory"
            :key="game.roundNumber"
            class="bg-gray-50 border border-gray-100 rounded-lg p-3 relative overflow-hidden shadow-sm"
          >
            <div
              class="absolute left-0 top-0 bottom-0 w-1.5"
              :class="game.winner === 1 ? 'bg-green-500' : 'bg-red-500'"
            ></div>

            <div class="pl-3">
              <div
                class="flex justify-between items-center mb-2 border-b border-gray-200 pb-2"
              >
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span
                    class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-gray-200 text-gray-600"
                  >
                    {{ game.pointsAwarded.player1 }} pts
                  </span>
                  <span
                    class="text-xs font-black uppercase"
                    :class="
                      game.winner === 1 ? 'text-green-600' : 'text-red-600'
                    "
                  >
                    {{ game.winner === 1 ? "WIN" : "LOSE" }}
                  </span>
                </div>
              </div>
              ...
              <div class="grid grid-cols-3 items-center text-center">
                <div class="flex flex-col">
                  ...
                  <span
                    ...
                    :class="
                      game.scoreDetail.player1 > game.scoreDetail.player2
                        ? 'text-gray-800'
                        : 'text-gray-400'
                    "
                  >
                ...
                <div class="flex flex-col">
                  ...
                  <span
                    class="text-xl font-black leading-none"
                    :class="
                      game.scoreDetail.player2 > game.scoreDetail.player1
                        ? 'text-gray-800'
                        : 'text-gray-400'
                    "
                  >
