<template>
    <div
        ...
        <img :src="biscaLogo" alt="Logo Bisca Royal" class="w-78  mb-0 opacity-95 drop-shadow-md relative top-1" />

        <div class="w-full max-w-md bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-black/10 p-6 -translate-y-10">
            <form @submit.prevent="submit" class="space-y-4">
                <label class="block">
                    <span class="text-sm font-medium text-black/80">Email</span>
                    <input v-model="email" type="email" required aria-label="Email" placeholder="meu@email.com"
                ...
                <label class="block">
                    ...
                    <input v-model="password" type="password" required aria-label="Password" placeholder="••••••••"
                        class="mt-1 block w-full rounded-xl border border-black/10 bg-white/60 px-4 py-3 placeholder-black/40 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition" />
                </label>

                <div class="flex items-center justify-between text-sm text-black/75">
                    <label class="inline-flex items-center gap-2">
                        <input type="checkbox" v-model="remember" class="rounded-sm mt-1 cursor-pointer" />
                        <span>Lembra-me</span>
                    </label>
                    <button type="button" class="text-black/70 underline hover:text-black/90 cursor-pointer" @click="forgot">
                        Esqueci a password
                    </button>
                </div>
            ...
            <footer class="mt-6 text-center text-xs text-black/60">
                ...
                <p>
                    Não tens conta?
                    <router-link to="/register" @click="signup" class="underline font-medium cursor-pointer ">Registar</router-link>
                </p>
            </footer>
        </div>
    </div>
