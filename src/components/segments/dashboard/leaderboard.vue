<template>
  <section class="snap-start w-full px-4 mt-6 lg:mt-12">
    <div class="flex flex-col items-center text-center w-full max-w-5xl mx-auto">
      <!-- Title -->
      <h2
        class="text-3xl sm:text-4xl lg:text-5xl font-serif tracking-widest text-rose-800 w-full text-center flex items-center justify-center gap-3">
        Leaderboard
      </h2>
      ...
      <div class="mt-6">
        <div
          class="inline-flex rounded-xl border-4 border-amber-700 bg-gradient-to-b from-amber-50 to-amber-100 shadow-md overflow-hidden">
          <button @click="switchMode('global')" :class="[
            'px-6 py-2 text-sm font-bold tracking-wide focus:outline-none cursor-pointer',
            viewMode === 'global'
          ...
          <button @click="switchMode('player')" :class="[
            'px-6 py-2 text-sm font-bold tracking-wide border-l-4 border-amber-700 focus:outline-none cursor-pointer',
            viewMode === 'player'
      ...
      <div v-if="viewMode === 'global'" class="mt-6 w-full max-w-xl">
        <div
          class="px-5 py-4 rounded-xl border-4 border-yellow-700 bg-gradient-to-b from-yellow-50 to-amber-100 shadow-md">
          <div class="flex items-center justify-center flex-wrap gap-2">
            <span class="text-xs font-bold uppercase tracking-wider text-yellow-900 mr-2">Sort:</span>
            <button v-for="key in sortOptions" :key="key.value" @click="toggleSort(key.value)" :class="[
              'px-3 py-1.5 rounded-full text-xs border font-medium cursor-pointer',
              sortKey === key.value
            ...
            <button v-if="sortKey" @click="clearSort"
              class="px-3 py-1.5 rounded-full text-xs border font-medium bg-white text-red-700 border-red-400 cursor-pointer">
              Reset
            </button>
          </div>
        </div>
      ...
      <div v-if="viewMode === 'player'" class="mt-10 w-full max-w-4xl">
        ...
        <div v-else class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <!-- Matches Played -->
            <div
              class="relative flex flex-col rounded-2xl bg-white border-4 border-blue-600 shadow-[0_4px_0_0_theme(colors.blue.800)]">
              <div
                class="px-4 py-2 flex items-center justify-between bg-blue-50 border-b-2 border-blue-200 rounded-t-xl">
              ...
              <div class="p-4 flex-1 flex flex-col justify-center">
                <div
                  class="w-12 h-12 mb-3 rounded-xl bg-blue-600 flex items-center justify-center border-2 border-blue-800 shadow-inner">
            ...
            <!-- Wins -->
            <div
              class="relative flex flex-col rounded-2xl bg-white border-4 border-green-600 shadow-[0_4px_0_0_theme(colors.green.800)]">
              <div
                class="px-4 py-2 flex items-center justify-between bg-green-50 border-b-2 border-green-200 rounded-t-xl">
              ...
              <div class="p-4 flex-1 flex flex-col justify-center">
                <div
                  class="w-12 h-12 mb-3 rounded-xl bg-green-600 flex items-center justify-center border-2 border-green-800 shadow-inner">
                  <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <!-- Win Rate -->
            <div
              class="relative flex flex-col rounded-2xl bg-white border-4 border-purple-600 shadow-[0_4px_0_0_theme(colors.purple.800)]">
              <div
                class="px-4 py-2 flex items-center justify-between bg-purple-50 border-b-2 border-purple-200 rounded-t-xl">
              ...
              <div class="p-4 flex-1 flex flex-col justify-center">
                <div
                  class="w-12 h-12 mb-3 rounded-xl bg-purple-600 flex items-center justify-center border-2 border-purple-800 shadow-inner">
                  <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-3 w-full bg-purple-200 rounded-full h-3 border-2 border-purple-500 overflow-hidden">
                  <div class="h-full bg-purple-600" :style="{ width: personalScore?.win_rate ?? '0%' }"></div>
                </div>
              </div>
            </div>
            ...
            <!-- Capote -->
            <div
              class="relative flex flex-col rounded-2xl bg-white border-4 border-orange-600 shadow-[0_4px_0_0_theme(colors.orange.800)]">
              <div
                class="px-4 py-2 flex items-center justify-between bg-orange-50 border-b-2 border-orange-200 rounded-t-xl">
              ...
              <div class="p-4 flex-1 flex flex-col justify-center">
                <div
                  class="w-12 h-12 mb-3 rounded-xl bg-orange-600 flex items-center justify-center border-2 border-orange-800 shadow-inner">
                  <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <!-- Bandeira -->
            <div
              class="relative flex flex-col rounded-2xl bg-white border-4 border-red-600 shadow-[0_4px_0_0_theme(colors.red.800)]">
              ...
              <div class="p-4 flex-1 flex flex-col justify-center">
                <div
                  class="w-12 h-12 mb-3 rounded-xl bg-red-600 flex items-center justify-center border-2 border-red-800 shadow-inner">
                  <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div v-if="viewMode === 'global'">
        <div>
          <button
            class="cursor-pointer text-sm font-bold tracking-wider text-yellow-900 mr-2 mx-auto px-2 py-2 rounded-xl border-4 border-yellow-700 mt-5 bg-gradient-to-b from-yellow-50 to-amber-100 shadow-md w-sm text-center"
            @click="seeMyPosition">
            Get my page
          </button>
        </div>
        <div class="mt-10 grid gap-6 w-full sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="(player, index) in leaderboardData.data" :key="player.nickname ?? player.id"
        ...
        <div v-if="leaderboardData?.last_page > 1 && viewMode === 'global'" class="flex justify-center mt-10 gap-4">
          <button
            ...
            :disabled="leaderboardData.current_page === 1" @click="changePage(leaderboardData.current_page - 1)"
          ...
          <button
            ...
            @click="changePage(leaderboardData.current_page + 1)" aria-label="Next page">
...
import { ChevronLeft, ChevronRight, Crown, Medal, Trophy } from 'lucide-vue-next'
...
async function seeMyPosition() {
  const leaderboard = await dashStore.getPositionByUserId(sortKey.value, currentUserId.value)
  //Otimizado para evitar chamadas desnecessárias
  console.log(leaderboard.data.current_page)
  fetchLeaderboard(leaderboard.data.current_page)
}
