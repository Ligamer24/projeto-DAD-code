...
const submitForm = async () => {
  ...
  try {
    ...
    const coinsToUser = response.data.value * 10
    const updatedUser = await apiStore.updateCoinsUser(coinsToUser)

    // Atualizar as coins do jogador
    const coinsObj = {
      transaction_datetime: new Date(),
      user_id: authStore.currentUser.id,
      coin_transaction_type_id: 2, //O type 2 é o Coin Purchase
      coins: coinsToUser,
    }

    const transaction = await apiStore.postCoinsTransaction(coinsObj)

    const coinsPurchaseObj = {
      purchase_datetime: new Date(),
      user_id: authStore.currentUser.id,
      coin_transaction_id: transaction.data.id,
      euros: response.data.value,
      payment_type: payment.value.type,
      payment_reference: payment.value.reference,
    }

    await apiStore.postCoinsPurchaseTransaction(coinsPurchaseObj)

    authStore.currentUser = updatedUser.data
...
<template>
  <div class="min-h-screen bg-lime-300 p-4">
    <ConfirmationModal :isOpen="isModalOpen" title="Confirm Purchase"
      :message="itemToBuy ? `Are you sure you want to buy ${itemToBuy.name} for ${itemToBuy.price} coins?` : ''"
      @confirm="confirmBuy" @cancel="closeBuyModal" />
    ...
    <div class="flex justify-end mt-4 mr-50">
      <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full"
        @click="mostrarModalAddCoins = true">
        Add coins to your account
      </button>
    </div>
    <div class="flex justify-end mt-4 mr-50">
      <router-link to="/transactionsUser"
      ...
      <div class="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <form @submit.prevent="submitForm" class="space-y-5">
          ...
          <div>
            ...
            <select v-model="payment.type" required
          ...
          <div>
            ...
            <input v-model="payment.reference" type="text" required :placeholder="currentPlaceholder"
              class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none">
          </div>

          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Value (1-99)</label>
            <input v-model.number="payment.value" type="number" min="1" max="99" step="1" required
              placeholder="Enter amount"
              class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none">
          </div>
          <p v-if="apiErrorMessage" class="mt-1 text-sm text-red-600 font-medium">
            Error: {{ apiErrorMessage }}
          </p>

          <div class="flex gap-3 pt-4">
            <button type="button" @click="mostrarModalAddCoins = false"
              class="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-semibold">
              Cancel
            </button>
            <button type="submit"
              class="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold shadow-md">
              Confirm
            </button>
          </div>
        </form>
    ...
    <div class="grid grid-cols-2 gap-4 mt-4">
      <div v-for="item in shopStore.items" :key="item.id"
        ...
        <div v-if="item.owned">
          <Button @click="selectCard(item.id)" :class="[
